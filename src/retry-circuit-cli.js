// Shared command body: the source package has a small JS entrypoint, while the native installer
// exposes the same command by symlinking hcode-supervise to the one hcode executable.
import { spawn } from "node:child_process";
import { once } from "node:events";
import { recordAttempt, publishCircuitStatus, resetCircuit } from "./retry-circuit.js";

export async function mainRetryCircuit(argv) {
  ...
  if (!stateFile || !statusFile) { console.error("usage: hcode-supervise --state FILE --status FILE [--session NAME] [--threshold 3] [--resume] -- COMMAND ..."); return 64; }
  if (argv.includes("--resume")) { resetCircuit(stateFile, statusFile); console.error(`circuit reset; resume with: ${resumeCommand}`); return 0; }
  const split = argv.indexOf("--"); const command = split >= 0 ? argv.slice(split + 1) : [];
  if (!command.length) { console.error("missing command after --"); return 64; }
  ...
  while (true) {
    ...
    if (state.state === "circuit-open") { console.error(`CIRCUIT OPEN: ${state.fingerprint} repeated ${state.consecutive}/${state.threshold}; work board red; session/objective preserved. Resume: ${resumeCommand}`); return 75; }
    if (state.state === "unobserved") { console.error(`UNOBSERVED: failure class is unknown; automatic retry stopped; session/objective preserved. Resume after inspection: ${resumeCommand}`); return 76; }
